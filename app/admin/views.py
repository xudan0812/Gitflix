...
def movie_add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Successfully added a movie", "ok")
...
def movie_edit(id=None):
    ...
    if request.method == "GET":
        ...
        form.tag.data = movie.tag
        form.star.data = movie.star
    if form.validate_on_submit():
        data = form.data
        movie_count = Movie.query.filter_by(title = data["title"]).count()
        if data["title"] != movie.title and movie_count == 1:
            flash("New name already existed", "err")
...
def preview_add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Successfully added a preview", "ok")
...
def preview_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        #     flash("New preview already existed", "err")
